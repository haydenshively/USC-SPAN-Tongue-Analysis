...
@film.processor
def find_motion(frame, prev_ret_val, i, MEMORY_LENGTH, DIFFERENCE_THRESH):
    if i is 0: return [History(MEMORY_LENGTH), None]
    else:
        history, motion = prev_ret_val

        gray = frame[:, :, 0]
        if history.time is 0:
            motion = np.zeros_like(gray).astype('float32')
        history += gray
